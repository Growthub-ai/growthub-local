...
Alongside the receipt stream sit two read-only observability rollups (additive, GET-only, secret-free, never-throwing — same shape as `GET /api/workspace/metadata-graph`): `GET /api/workspace/health` returns one actionable health summary (`status`, `issues[]`, `metrics`) derived from stale widgets, missing/empty live sources, dangling references, and pipeline run state; `GET /api/workspace/agent-context` returns the compact packet a first-session agent reads to understand the workspace in one request (summary, capabilities, health critical-state slice, entrypoints). Both are derived from the metadata store/graph (`lib/workspace-health.js`) — they add no mutation lane, persistence layer, or PATCH field. Reference: [`docs/WORKSPACE_HEALTH_AGENT_CONTEXT_V1.md`](./docs/WORKSPACE_HEALTH_AGENT_CONTEXT_V1.md).
