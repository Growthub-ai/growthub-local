...
Together, scope (assignment packets + `x-growthub-app-scope`), the mutation boundary, the outcome-receipt stream, and continuity form the **Bounded Agency Contract** — the formal agent↔workspace agreement, its conformance checklist, and the sprint roadmap that extends it are specified in [`docs/BOUNDED_AGENCY_CONTRACT_V1.md`](./docs/BOUNDED_AGENCY_CONTRACT_V1.md).
