...
- `docs/CODEX_PLUGIN_MARKETPLACE_V1.md` — the installable Codex plugin/marketplace surface (`.agents/plugins/` + `plugins/`) that packages the same governed console for Codex app/CLI/IDE users.
...
**Unified API Resolver Registry V1 (API contract v1.5.1)** ties every resolver to the governed `api-registry` record it serves. A resolver is the workspace's provider-agnostic "API → governed rows" abstraction; 1.5.1 makes it dynamic and traceable without destabilizing the API Registry contract: `GET /api/workspace/resolvers` returns an additive `registry` index correlating each record to its resolver (connectorKind, provenance, file, registered/tested, shape, score, next action, endpoint); the no-code cockpit **constructs** the governed resolver from the tested response shape (`lib/resolver-constructor.js`) instead of asking the user to fill open fields; and every registered resolver is an addressable, app-scoped Next.js endpoint at `/api/resolvers/<integrationId>`. Generated resolver code and the `_registry.generated.json` / `_endpoints.generated.json` artifacts are **projections of the governed record — never hand-edited** (do-not-edit banner + the `scripts/check-resolver-registry.mjs` CI drift guard). Contract: `@growthub/api-contract/resolver-registry`. Reference: [`docs/UNIFIED_API_RESOLVER_REGISTRY_CONTRACT_V1_5_1.md`](./docs/UNIFIED_API_RESOLVER_REGISTRY_CONTRACT_V1_5_1.md) and the completed release snapshot [`docs/UNIFIED_API_RESOLVER_REGISTRY_RELEASE_SNAPSHOT_V1.md`](./docs/UNIFIED_API_RESOLVER_REGISTRY_RELEASE_SNAPSHOT_V1.md).

**Governed Compute Realization V1 (API contract v1.8)** makes WHERE a custom-model workload executes a replaceable governed realization of the existing lifecycle — never a second universe. Provider identity is an ordinary `api-registry` row (`metadata.computeProvider`, schema `growthub-compute-provider-v1`, env-var NAMES only — credential-shaped values invalidate the row); requirements derive from the SAME adaptive student plan + preflight evidence (one sizing ladder, no second hardware detector) into seven vendor-free Capacity Profiles; the pure deterministic resolver (`lib/compute-resolver.js`) gates fail-closed (unknown cost is never zero under a hard budget) and ranks with a stable provider-id tiebreak; execution stays on `POST /api/workspace/sandbox-run` (additive provider-compute branch — there is no `/api/compute/run`), with evidence-gated normalized events, fail-closed allocation idempotency, artifact honesty (provider "completed" without a sha256-identified artifact is non-promotable), and release honesty (`capacityMayStillExist`/`costMayAccrue` stay durably visible). Promotion remains evaluation-only; Mothership routing is untouched. Read surface: `GET /api/workspace/compute` (read-only by construction). Adapters (`lib/adapters/compute/`): local-machine, runpod-pods, modal-functions, ray-cluster → aws-hyperpod-ray / coreweave-ray — cluster procurement/billing stays GH App authority. SDK: `@growthub/api-contract/compute`. Reference: [`docs/GOVERNED_COMPUTE_REALIZATION_V1.md`](./docs/GOVERNED_COMPUTE_REALIZATION_V1.md) and the banked proof set [`docs/proofs/governed-compute-realization/`](./docs/proofs/governed-compute-realization/PROOF-INDEX.md).
...
The **Intelligence layer** is `GET /api/workspace/metadata-graph` (typed node/edge world model, also rendered by the Workspace Map) plus the pure derivers in `apps/workspace/lib/` — `deriveBlastRadius` (the transitive reverse-dependency closure over the graph's single-hop `findDependents`) and its compounding family: `deriveStaleSurfaces`, `deriveWorkflowImpact`, `deriveProvenanceLineage`, `deriveAppReadiness`, `deriveContractCompliance`, and `derivePatchImpact` (the one shared add/modify/**remove** impact model used by the preflight route, `growthub patch`, and MCP alike). Validate workspace changes by exporting and operating the live runtime (`node scripts/export-seed-workspace.mjs`), not by hand-built fixtures.

The **agent-facing console** over this plane is `growthub serve --mcp`: **read + dry-run + governed hand-off, never a mutation tool.** Intelligence tools are read-only; `preflight_patch` dry-runs against Law (proxies the authoritative runtime preflight in `--live`, forwards `appScope`, never writes); `next_actions` emits the exact governed call — there is no third mutation path, and `--live` rehydrates per call so reads are never stale. The simplest rule: **use MCP to answer "what exists, what depends on what, what would break, is it ready, and what governed call happens next" — never to change reality.** Canonical contract: [`docs/GOVERNED_MCP_CONSOLE_V1.md`](./docs/GOVERNED_MCP_CONSOLE_V1.md).
