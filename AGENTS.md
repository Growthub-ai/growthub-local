...
**Workspace Authority Intelligence V1** is the converged read-only operator intelligence layer over config + source records + metadata graph + the agent-outcomes receipt stream — it answers "is this workspace safe, healthy, understandable, and governed enough for agents to act?" in one place. It unifies three projections: workspace **health** (`status`/`issues`/`metrics` from stale widgets, missing/empty live sources, dangling references, pipeline run state), the **agent context** packet (summary, capabilities, health critical-state slice, entrypoints), and **governance causation** (same-actor route-shopping correlated from receipts). The two machine-readable read models are GET-only, secret-free, never-throwing (same shape as `GET /api/workspace/metadata-graph`): `GET /api/workspace/health` and `GET /api/workspace/agent-context`. The pure combiner `lib/workspace-authority-intelligence.js` (`deriveWorkspaceAuthorityIntelligence`, `deriveAuthorityStatus`, `deriveAuthorityNextActions`) rolls health + governance into one status (`clear`/`watch`/`attention`) and one normalized next-action model that only ever points at existing fix surfaces (`/data-model`, `/`, `/workflows`, swarm-run). The single operator surface is `WorkspaceAuthorityCockpit.jsx`, reachable via the read-only `/governance` slash command (`view: "authority"`) and the CEO cockpit › Authority tab — both render the same component over the same model. No new mutation lane, PATCH field, object type, persistence, or runtime. Reference: [`docs/WORKSPACE_AUTHORITY_INTELLIGENCE_V1.md`](./docs/WORKSPACE_AUTHORITY_INTELLIGENCE_V1.md).
