...
| --- | --- | --- |
| Live Runpod pod/serverless allocation | **Unexecuted — harness ready** | The live proof harness (`scripts/e2e-compute-runpod-live-proof.mjs`, offline self-test green) executes the governed transport + registered adapter against the real control plane, observation-only over an operator-deployed pod, with secret-free banked evidence and golden screenshots — see [`live-runpod/PROOF.md`](./live-runpod/PROOF.md). Execution requires a direct-egress network (the DNS-pinned transport cannot ride a CONNECT proxy) plus `RUNPOD_API_KEY`/`RUNPOD_LIVE_POD_ID`. The adapter, conformance suite (11 tests incl. e2e over faked transport), failure/recovery behavior, and idempotent reconcile-by-name are complete. |
