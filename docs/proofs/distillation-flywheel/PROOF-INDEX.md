...
| util-08 | Workflow canvas | `util-08-workflow-canvas.png` | `util-08-workflow-canvas-readback.json` | the one-click product on the REAL canvas: `custom-model-chat (3/3) · v2 · live`, all nodes **Completed** from actual runs, the api-registry node selected with its config sidecar open — **Connected**, POST `/chat/completions`, auth by reference (no secret), **Delta tags** (`routing, input, output`) — the governed sandbox object record rendered as the workflow, binding the atomic api-registry record | derived |
| util-09 | The loop, visibly | `util-09-loop-growth-cockpit.png` | `util-09-loop-growth-cockpit-readback.json` | one more REAL workflow run grows the harvest on disk (43 → 44 traces, reasoning present), the sidecar + trace-capture receipt refresh, and the cockpit's Harvest chip re-renders the grown count — use → harvest → next generation, the behavior loop on screen | live-http |
