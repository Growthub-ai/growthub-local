...
| **E2E probe scripts** | Manual curl only | Bundled HTTP probe scripts for `/api/workspace` and `sandbox-run` |
...
## 19. Repository automation (golden path + distillation lane)

These scripts ship in the monorepo root under `scripts/`:

| Script | Purpose |
|---|---|
| `scripts/awac-golden-path-probe.mjs` | PLG KPI wrapper: runs `awac-workspace-api-probe.mjs` and prints wall-clock ms to stderr |
| `scripts/awac-workspace-api-probe.mjs` | Full-stack HTTP assertions (GET/PATCH workspace, reference-options, sandbox-run) |
| `scripts/export-distillation-jsonl.mjs` | Reads `growthub.source-records.json`, filters `adapter === "local-intelligence"` receipts, writes JSONL for external fine-tuning |
