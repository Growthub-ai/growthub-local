...
| **scaffolding** | `docs/`, `scripts/`, root contracts (`README.md`, `ARCHITECTURE.md`, `AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING.md`, `.cursorrules`), `.github/`, `.githooks/`, `.claude/`, `.agents/`, `.claude-plugin/` + `claude-plugins/` (Claude Code plugin marketplace surface — see `docs/CLAUDE_CODE_PLUGIN_MARKETPLACE_V1.md`), root config (`package.json`, `tsconfig*`, `pnpm-workspace.yaml`, `pnpm-lock.yaml`) | Tooling, contracts, docs, CI. | Freely editable here; keep docs aligned (`ARCHITECTURE.md` Documentation Contract). |
