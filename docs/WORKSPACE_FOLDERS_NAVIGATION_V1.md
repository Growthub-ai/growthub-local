...
- The official `growthub-custom-workspace-starter-v1` export may ship with default client-facing folders seeded in the hidden `nav-folders` object (see `docs/CREATIVE_OS_STARTER_SEED_V1.md`). Users can rename, reorder, or replace those folders from the rail without a separate persistence route.
...
This feature is part of the `@growthub/cli@0.13.2` / `@growthub/create-growthub-local@0.13.2` workspace starter export. Per the dist rebuild protocol, source and asset changes land first; the committed CLI dist is rebuilt from the full workspace in the super-admin release lane before npm publication.
