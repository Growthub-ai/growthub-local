#!/usr/bin/env bash
# SessionStart orientation for governed Growthub workspaces in Codex.
...
Use the governed-universe MCP tools (describe_workspace, find_downstream_dependencies, simulate_causal_impact, preflight_patch, next_actions) to read, reason, and hand off; see the Growthub Governed Console Codex plugin skills (governed-console, governed-workspace-mutation) before mutating anything.
EOF
exit 0
