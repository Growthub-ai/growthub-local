...
function resolveLocalDirectoryPath(rawPath: string | null | undefined, missingMessage: string) {
  ...
  if (!existsSync(resolvedPath)) {
    return { error: `Local folder "${resolvedPath}" does not exist on this machine.` } as const;
  }
  if (!statSync(resolvedPath).isDirectory()) {
    return { error: `Local folder "${resolvedPath}" is not a directory.` } as const;
  }
  return { path: resolvedPath } as const;
}

function resolveIssueBranchPath(workspace: { providerRef: string | null; cwd: string | null } | null) {
  return resolveLocalDirectoryPath(
    workspace?.providerRef ?? workspace?.cwd ?? null,
    "This issue does not have a bound local branch/worktree yet.",
  );
}

function extractLinkedPrMetadata(raw: unknown): { repo: string | null; branch: string | null } {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return { repo: null, branch: null };
  }
  const metadata = raw as Record<string, unknown>;
  const linkedPr =
    metadata.linkedPr && typeof metadata.linkedPr === "object" && !Array.isArray(metadata.linkedPr)
      ? (metadata.linkedPr as Record<string, unknown>)
      : null;
  return {
...
function repoMatchesLinkedPr(project: {
  name: string;
  codebase: { repoUrl: string | null; repoName: string | null };
}, repo: string | null) {
...
export function issueRoutes(db: Db, storage: StorageService) {
  ...
  router.post("/issues/:id/open-local", async (req, res) => {
    ...
    let openedPathResult = currentExecutionWorkspace
      ...
      : ({ error: "missing execution workspace" } as const);
    let createdWorkspace = false;
    let cursorOpenPath: string | null = null;

    if ("error" in openedPathResult) {
      let resolvedProjectId = issue.projectId;
      let preferredBranchName: string | null = null;
      ...
      if (preferredBranchName) {
        const existingStrategy =
          config.workspaceStrategy && typeof config.workspaceStrategy === "object" && !Array.isArray(config.workspaceStrategy)
            ? (config.workspaceStrategy as Record<string, unknown>)
            : {};
      ...
      if (
        issue.executionWorkspaceId !== currentExecutionWorkspace.id ||
    ...
    const cursorTarget = resolveLocalDirectoryPath(
      cursorOpenPath ?? openedPathResult.path,
      "Paperclip could not determine the local project folder to open in Cursor.",
    );
