...
function redactApprovalComment(comment) {
    return {
        ...comment,
        body: redactCurrentUserText(comment.body),
    };
}
export function approvalService(db) {
    const agentsSvc = agentService(db);
    const budgets = budgetService(db);
    const canResolveStatuses = new Set(["pending", "revision_requested"]);
    const resolvableStatuses = Array.from(canResolveStatuses);
    async function getExistingApproval(id) {
    ...
    async function resolveApproval(id, targetStatus, decidedByUserId, decisionNote) {
        ...
        if (!canResolveStatuses.has(existing.status)) {
            ...
            throw unprocessable(`Only pending or revision requested approvals can be ${targetStatus === "approved" ? "approved" : "rejected"}`);
        }
        const now = new Date();
    ...
    return {
        ...
        reject: async (id, decidedByUserId, decisionNote) => {
        ...
        addComment: async (approvalId, body, actor) => {
            const existing = await getExistingApproval(approvalId);
