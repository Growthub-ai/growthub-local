...
function deriveColorFromSeed(seed) {
    let hash = 0;
...
function createFaviconDataUrl(background, foreground) {
    const svg = [
...
export function isWorktreeUiBrandingEnabled(env = process.env) {
    return isTruthyEnvValue(env.PAPERCLIP_IN_WORKTREE);
}
export function getWorktreeUiBranding(env = process.env) {
...
export function renderRuntimeBrandingMeta(branding, surfaceProfile = "dx") {
    const profileMeta = `<meta name="paperclip-surface-profile" content="${escapeHtmlAttribute(surfaceProfile)}" />`;
...
export function applyUiBranding(html, env = process.env, surfaceProfile = "dx") {
    ...
    return replaceMarkedBlock(withFavicon, RUNTIME_BRANDING_BLOCK_START, RUNTIME_BRANDING_BLOCK_END, renderRuntimeBrandingMeta(branding, surfaceProfile));
}
//# sourceMappingURL=ui-branding.js.map