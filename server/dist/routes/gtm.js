...
function hasChromeEnabledAdapter(agent) {
    return readRecord(agent.adapterConfig)?.chrome === true;
}
async function resolveIssueBoundInvokeTarget(issues, companyId, agentId, requestedIssueId) {
    if (requestedIssueId) {
...
function gtmDirectoryAgents(rows) {
...
function gtmAgentMetadata(metadata) {
    ...
    if (!Object.prototype.hasOwnProperty.call(merged, "skills")) {
        ...
        merged.skills = merged.skills.filter((s) => typeof s === "string" && s.length > 0);
    }
    return merged;
}
...
function sanitizeBrowserIdentitySegment(value, fallback) {
...
function buildBrowserIsolationDefaults(input) {
...
function mergeBrowserIsolationConfig(input) {
    ...
    const browserSlot = typeof merged.browserSlot === "string" && merged.browserSlot.trim().length > 0
    ...
    const tabGroupKey = typeof merged.tabGroupKey === "string" && merged.tabGroupKey.trim().length > 0
    ...
    const tabGroupLabel = typeof merged.tabGroupLabel === "string" && merged.tabGroupLabel.trim().length > 0
    ...
    const crossAgentTabPolicy = typeof merged.crossAgentTabPolicy === "string" && merged.crossAgentTabPolicy.trim().length > 0
...
function buildDraftTitle(prompt, draftProfile) {
...
function mergeGtmAgentAdapterConfig(adapterType, config, options) {
    const merged = { ...config };
...
function applyBrowserBridgeCors(req, res) {
...
export function gtmRoutes(db) {
    ...
    router.post("/companies/:companyId/workspace-config/claude-browser", async (req, res) => {
        ...
        const rows = gtmDirectoryAgents(await agents.list(companyId));
        const existingAgent = rows.find(isManagedClaudeBrowserAgent) ?? null;
        const adapterConfig = mergeGtmAgentAdapterConfig("claude_local", {
    ...
    router.post("/companies/:companyId/agents", async (req, res) => {
        ...
        const rawAdapterConfig = typeof req.body?.adapterConfig === "object" && req.body?.adapterConfig !== null
        ...
        const created = await agents.create(companyId, {
            ...
            metadata: (() => {
                ...
                const kind = typeof reqMeta.gtmKind === "string" && reqMeta.gtmKind.trim() ? reqMeta.gtmKind.trim() : "gtm_user_created";
    ...
    router.post("/agents/:agentId/invoke", async (req, res) => {
        ...
        let payload = null;
        let contextSnapshot = { source: "gtm.agent.invoke" };
        let wakeReason = "gtm_manual_invoke";
        let wakeSource = "on_demand";
        if (chromeAgent) {
            const targetIssue = await resolveIssueBoundInvokeTarget(issues, companyId, agent.id, requestedIssueId);
            if (!targetIssue) {
                res.status(409).json({
                    error: "Chrome agents must be launched on an assigned runnable issue. Assign or reopen a GTM issue for this agent, then dispatch that issue instead of using a free-run invoke.",
                });
                return;
            }
            payload = {
    ...
    router.post("/agents/:agentId/pause", async (req, res) => {
        ...
        await heartbeats.cancelActiveForAgent(agentId, "GTM: agent paused from workspace (active invocations cancelled)");
        res.json(updated);
    });
    ...
    router.post("/agents/:agentId/stop-runs", async (req, res) => {
        ...
        const cancelledCount = await heartbeats.cancelActiveForAgent(agentId, "GTM: stopped runs from workspace (invocations cancelled)");
    ...
    router.post("/companies/:companyId/campaign-drafts", async (req, res) => {
        ...
        res.json({
            ...
            targetAudience: "",
            offer: "",
            successDefinition: existingCampaignCount > 0
    ...
    router.post("/companies/:companyId/tickets", async (req, res) => {
        ...
        assertCompanyAccess(req, companyId);
        const actor = getActorInfo(req);
        const stageDefinitions = Array.isArray(req.body?.stageDefinitions) && req.body.stageDefinitions.length > 0
            ? req.body.stageDefinitions
            : undefined;
        const stageOrder = Array.isArray(req.body?.stageOrder) && req.body.stageOrder.length > 0
