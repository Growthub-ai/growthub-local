...
export function GtmAgentModal({ open, onClose, companyId, onSuccess }: GtmAgentModalProps) {
  ...
  function handleSendToCeo() {
    ...
    const parts: string[] = [];
    parts.push(`Draft focus: ${profileLabel}`);
    if (aiPrompt.trim()) parts.push(aiPrompt.trim());
    if (aiExtendExisting) parts.push("Extend current GTM patterns — bias toward the existing GTM agent posture instead of a net-new structure.");
