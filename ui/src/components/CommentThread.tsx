...
import { ArrowUpRight, Check, ChevronDown, ChevronUp, Copy, Paperclip } from "lucide-react";
...
function CollapsibleComment({
  ...
  const [collapsed, setCollapsed] = useState(isUserComment);
    ...
    >
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="flex items-center gap-1.5">
          ...
          {isUserComment && <CollapseButton collapsed={collapsed} onToggle={() => setCollapsed((c) => !c)} />}
        </span>
      </div>
      {!collapsed && (
        <>
          ...
          return (
            <div key={`run:${run.runId}`} className="border border-border bg-accent/20 p-3 overflow-hidden min-w-0 rounded-sm">
              ...
              <div className="flex items-center justify-between gap-2 text-xs">
                ...
                <Link
                  to={`/agents/${run.agentId}/runs/${run.runId}`}
                  className="inline-flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors rounded px-1.5 py-1 hover:bg-accent/40"
                  title="View run"
                >
