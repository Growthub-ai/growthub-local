...
function OverviewContent({
  ...
  goals?: import("@paperclipai/shared").Goal[];
...
export function ProjectDetail() {
  ...
  const { data: allGoals } = useQuery({
    queryKey: queryKeys.goals.list(resolvedCompanyId ?? "__none__"),
    queryFn: () => goalsApi.list(resolvedCompanyId!),
    enabled: !!resolvedCompanyId && activeTab === "overview",
    staleTime: 30_000,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "overview"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
