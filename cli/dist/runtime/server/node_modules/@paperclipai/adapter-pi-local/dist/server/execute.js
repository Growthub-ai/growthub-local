...
async function ensurePiSkillsInjected(onLog) {
    ...
    if (skillsEntries.length === 0)
        return;
    const removedSkills = await removeMaintainerOnlySkillSymlinks(PI_AGENT_SKILLS_DIR, skillsEntries.map((entry) => entry.name));
    ...
    for (const entry of skillsEntries) {
        ...
        try {
            ...
            if (result === "skipped")
                continue;
            await onLog("stderr", `[paperclip] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.name}" into ${PI_AGENT_SKILLS_DIR}\n`);
        }
        catch (err) {
            await onLog("stderr", `[paperclip] Failed to inject Pi skill "${entry.name}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`);
        }
    }
}
