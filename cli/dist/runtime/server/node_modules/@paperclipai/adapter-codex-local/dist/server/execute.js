...
import { asString, asNumber, asBoolean, asStringArray, parseObject, buildPaperclipEnv, redactEnvForLogs, ensureAbsoluteDirectory, ensureCommandResolvable, ensurePaperclipSkillSymlink, ensurePathInEnv, listPaperclipSkillEntries, removeMaintainerOnlySkillSymlinks, renderTemplate, joinPromptSections, runChildProcess, } from "@paperclipai/adapter-utils/server-utils";
...
async function isLikelyPaperclipRuntimeSkillSource(candidate, skillName) {
...
export async function ensureCodexSkillsInjected(onLog, options = {}) {
    ...
    const removedSkills = await removeMaintainerOnlySkillSymlinks(skillsHome, skillsEntries.map((entry) => entry.name));
    for (const skillName of removedSkills) {
        await onLog("stdout", `[paperclip] Removed maintainer-only Codex skill "${skillName}" from ${skillsHome}\n`);
    }
        ...
        try {
            ...
            if (existing?.isSymbolicLink()) {
                ...
                if (resolvedLinkedPath &&
                    ...
                    await onLog("stdout", `[paperclip] Repaired Codex skill "${entry.name}" into ${skillsHome}\n`);
                    continue;
                }
            }
            ...
            if (result === "skipped")
                continue;
            await onLog("stdout", `[paperclip] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.name}" into ${skillsHome}\n`);
        }
        catch (err) {
...
export async function execute(ctx) {
    ...
    await ensureCodexSkillsInjected(onLog, effectiveCodexHome ? { skillsHome: path.join(effectiveCodexHome, "skills") } : {});
