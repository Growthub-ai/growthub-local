...
import { coerceGtmState, createDefaultGtmState, toGtmViewModel, } from "@paperclipai/shared";
...
function normalizeWorkflowRunState(state) {
...
export function readGtmState() {
    const filePath = resolveGtmStatePath();
    const state = fs.existsSync(filePath)
        ? coerceGtmState(JSON.parse(fs.readFileSync(filePath, "utf-8")))
        : createDefaultGtmState();
    return normalizeWorkflowRunState(state);
}
export function writeGtmState(state) {
    const filePath = resolveGtmStatePath();
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, JSON.stringify(state, null, 2) + "\n", "utf-8");
}
export function readGtmViewModel() {
    return toGtmViewModel(readGtmState());
}
export function launchLocalGtmWorkflow() {
...
//# sourceMappingURL=gtm-state.js.map