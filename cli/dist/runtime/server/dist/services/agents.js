...
function buildConfigSnapshot(row) {
    const adapterConfig = typeof row.adapterConfig === "object" && row.adapterConfig !== null && !Array.isArray(row.adapterConfig)
...
export function agentService(db) {
    ...
    async function getMonthlySpendByAgentIds(companyId, agentIds) {
    ...
    async function ensureManager(companyId, managerId) {
    ...
    async function assertCompanyShortnameAvailable(companyId, candidateName, options) {
        const candidateShortname = normalizeAgentUrlKey(candidateName);
        if (!candidateShortname)
            return;
