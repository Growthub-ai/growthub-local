...
import { createNativeIntelligenceBackend, createNativeIntelligenceProvider, checkBackendHealth, readIntelligenceConfig, writeIntelligenceConfig, buildMarketingContext, buildDeterministicContext, runLocalIntelligenceSandboxTask, buildLocalIntelligenceSandboxPrompts, sandboxEnvelopeToTraceRecord, formatTraceRecordLine, } from "./runtime/native-intelligence/index.js";
...
import { addObservation, addSummary, searchMemory, searchSummaries, listMemoryProjects, getMemoryStats, getObservations, buildMemoryContext, buildSemanticContext, canSync, readProviderConfig, writeProviderConfig, inspectMemoryProfileBinding, setAutoSyncEnabled, syncProjectToProfile, autoSyncProjectIfReady, pullProjectMemoriesIfAvailable, } from "./runtime/memory/index.js";
import { promptExtendedProvider } from "./prompts/llm.js";
const program = new Command();
const DATA_DIR_OPTION_HELP = "Growthub data directory root (isolates local instance state)";
...
function isDiscoveryAuthenticated() {
...
async function runNativeIntelligenceHub() {
    while (true) {
        ...
        if (action === "models") {
            ...
            if (p.isCancel(adapterChoice) || adapterChoice === "__back_to_local_intel")
                continue;
            ...
            if (p.isCancel(applyConfirmed) || !applyConfirmed)
                continue;
            ...
            if (!health.available) {
                ...
                p.note([...(health.error ? [`Error: ${health.error}`] : []), "You can still run prompt flow and retry health later."].join("\n"), "Local model status");
                continue;
            }
        ...
        if (action === "provider-config") {
            ...
            p.note(`Provider: ${result.provider}\nModel: ${result.modelId ?? "default"}\nAPI key: ${result.apiKey ? "configured" : "not needed"}`, "Provider config saved (intelligence + memory)");
            continue;
        }
...
async function runLocalIntelligenceSandboxDiscovery(baseUrl, defaultModel) {
    ...
    try {
        ...
        if (!p.isCancel(exportChoice) && exportChoice) {
            ...
            const line = formatTraceRecordLine(sandboxEnvelopeToTraceRecord(envelope, {
...
async function runLocalPromptChat(baseUrl, defaultModel) {
    ...
    while (true) {
        ...
        try {
            const out = await completeWithRetry(backend, baseConfig, {
...
function extractFactsFromResponse(text) {
...
function saveLocalThread(thread) {
    const dir = resolveLocalThreadsDir();
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(thread.filePath, `${JSON.stringify({ id: thread.id, messages: thread.messages }, null, 2)}\n`, "utf-8");
    const activePath = path.resolve(dir, "active-thread.json");
    fs.writeFileSync(activePath, `${JSON.stringify({ id: thread.id, messages: thread.messages }, null, 2)}\n`, "utf-8");
}
...
async function runMemoryKnowledgeHub(opts) {
    ...
    if (!isDiscoveryAuthenticated()) {
        ...
        if (p.isCancel(connectChoice) || connectChoice === "__back_to_hub")
            return "back";
    ...
    while (true) {
        ...
        if (p.isCancel(action) || action === "__back_to_hub") {
            ...
            try {
                ...
                if (auto.ran && auto.result?.status === "ok" && (auto.result.pushedObservations + auto.result.pushedSummaries) > 0) {
                    p.log.message(pc.dim(`⚡ Auto-synced ${auto.result.pushedObservations} obs + ${auto.result.pushedSummaries} summaries to Growthub.`));
                }
            }
            catch {
        ...
        if (action === "timeline") {
            ...
            if (observations.length === 0) {
                p.note("No observations stored yet. Start a local prompt chat to begin capturing memories.", "Timeline");
                continue;
            }
            const lines = [];
            let lastDate = "";
        ...
        if (action === "provider") {
            ...
            p.note(`Provider: ${result.provider}\nModel: ${result.modelId ?? "default"}\nAPI key: ${result.apiKey ? "configured" : "not needed"}`, "Provider config saved");
            continue;
        }
        if (action === "sync") {
            ...
            if (result.status === "ok") {
                syncSpinner.stop(`Synced ${result.pushedObservations} observations and ${result.pushedSummaries} summaries.`);
            }
        ...
        if (action === "auto-sync") {
            ...
            if (next) {
                ...
                if (result.status === "ok") {
                    ...
                    p.note(`Auto-sync enabled, but the initial push could not complete: ${result.reason ?? "unknown"}.`, "⚡ Auto-sync · ON");
                }
            }
            else {
        ...
        if (action === "pull") {
            ...
            if (result.status === "ok") {
                pullSpinner.stop(`Pulled ${result.pulledObservations} observations and ${result.pulledSummaries} summaries.`);
            }
            else if (result.status === "unavailable") {
...
async function runGovernedWorkspaceAgentsFlow() {
    ...
    while (true) {
        ...
        if (action === "bindings") {
            ...
            if (!fork)
                continue;
            const result = await import("./commands/bridge.js").then((mod) => mod.listHostedAgentBindings({ forkId: fork.forkId }));
            p.note([
                ...
                "",
                ...result.bindings.map((binding) => `${binding.agentSlug} · ${binding.agentName ?? ""} · execution=gh-app · localExecution=false`),
                "",
        ...
        if (action === "unbind") {
            ...
            if (!fork)
                continue;
            const result = await import("./commands/bridge.js").then((mod) => mod.removeHostedAgentBinding({ agentSlug: slug, forkId: fork.forkId }));
            p.note([
                `Agent: ${slug}`,
                `Workspace: ${fork.label ?? fork.forkId}`,
...
async function runCreateGovernedWorkspaceFlow(opts) {
    workspaceLoop: while (true) {
        ...
        if (starterChoice === "import-github") {
            ...
            if (p.isCancel(repoRaw) || !repoRaw)
                continue workspaceLoop;
            const { promptForInteractiveWorkspacePath, startSourceImportFlow, } = await import("./commands/source-import-discovery.js");
...
async function runDiscoveryHub(opts) {
    ...
    while (true) {
        ...
        if (surfaceChoice === "workspace-ops") {
            ...
            const looksLikeWorkspace = (candidate) => fs.existsSync(path.resolve(candidate, ".growthub-fork")) ||
            ...
            if (!target) {
                ...
                if (p.isCancel(pick) || pick === "__back")
                    continue;
                target = String(pick);
            }
            ...
            while (stayInOps) {
                ...
                const tick = (ok) => (ok ? pc.green("✓") : pc.dim("○"));
                ...
                if (status.issues.length > 0) {
                    ...
                    for (const issue of status.issues.slice(0, 4))
                ...
                if (action === "qa") {
                    ...
                    const icon = (s) => s === "pass" ? pc.green("✓") : s === "fail" ? pc.red("✗") : s === "warn" ? pc.yellow("!") : pc.dim("○");
                    const qaLines = [];
                ...
                if (action === "deploy-check") {
                    runChild(["workspace", "deploy", "check", "--fork", target]);
                    continue;
                }
                if (action === "deploy-env") {
                    runChild(["workspace", "deploy", "vercel", "--print-env", "--fork", target]);
                    continue;
                }
                if (action === "upstream-check") {
                    runChild(["workspace", "upstream", "check", "--fork", target]);
                    continue;
                }
                if (action === "upstream-heal") {
                    runChild(["workspace", "upstream", "heal", "--dry-run", "--fork", target]);
                    continue;
                }
                if (action === "surface-list") {
                    runChild(["workspace", "surface", "list", "--fork", target]);
                    continue;
                }
                if (action === "portal-prepare") {
                    const slug = await p.text({ message: "Client slug to prepare (e.g. acme-co):", placeholder: "acme-co" });
                    if (p.isCancel(slug) || !slug)
                        continue;
