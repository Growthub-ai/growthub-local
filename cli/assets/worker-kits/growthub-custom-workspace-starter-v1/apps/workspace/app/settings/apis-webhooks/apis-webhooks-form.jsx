...
function ApisWebhooksForm({ persistence, refs }) {
  ...
  async function save(event) {
    ...
    try {
      ...
      const envWritten = Array.isArray(payload.envLocalWritten) ? payload.envLocalWritten : [];
      setMessage(envWritten.length
        ? `Saved. Wrote ${envWritten.length} secret${envWritten.length === 1 ? "" : "s"} to .env.local (names only in config).`
        : "Saved.");
    } catch (error) {
