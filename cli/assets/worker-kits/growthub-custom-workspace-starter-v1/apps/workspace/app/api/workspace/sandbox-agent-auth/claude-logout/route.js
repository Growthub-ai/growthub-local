...
import { runClaudeSandboxLogout } from "@/lib/sandbox-agent-auth";
...
async function POST(request) {
  ...
  const result = await runClaudeSandboxLogout({ objectId, name });
  if (!result.ok && !result.payload) {
    return NextResponse.json({ ok: false, error: result.error }, { status: result.status || 500 });
  }
  return NextResponse.json(
    result.payload || { ok: false, error: result.error },
