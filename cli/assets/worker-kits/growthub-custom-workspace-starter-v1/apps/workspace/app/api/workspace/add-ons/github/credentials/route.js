...
async function POST(request) {
  ...
  const token = body?.useLocalSession
    ? await readLocalGithubToken()
    : clean(body?.credentials?.token ?? body?.token);
  if (!token) {
    return jsonError(
      body?.useLocalSession
        ? "No local GitHub CLI session token was available. Sign in with `gh auth login` or paste a repo-scoped token."
