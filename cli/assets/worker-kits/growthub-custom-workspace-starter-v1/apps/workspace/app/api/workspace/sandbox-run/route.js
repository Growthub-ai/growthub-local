...
import { readServerSecretEntry, resolveEnvRefStatuses } from "@/lib/workspace-env-resolver";
...
async function POST(request) {
  ...
  const env = {};
  const { resolved: envRefsResolved, missing: envRefsMissing } = resolveEnvRefStatuses(envRefSlugs);
  for (const slug of envRefsResolved) {
    const entry = readServerSecretEntry(slug);
