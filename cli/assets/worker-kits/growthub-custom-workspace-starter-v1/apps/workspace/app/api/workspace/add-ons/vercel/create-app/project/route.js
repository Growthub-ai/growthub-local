 ...
 * guidance).
 *
 * Data Model contract: this updates the same `vercel-projects` row created by
 * the GitHub repo step; deployment proof is added by the deploy route.
 */
...
async function POST(request) {
  ...
  project = {
    ...
    gitRepo: project.gitRepo || repoFullName,
  };

  const now = new Date().toISOString();
  const currentConfig = await readWorkspaceConfig();
  const withProjectRow = withVercelProjectsDirectory(currentConfig, {
    projects: [project],
    linkedAt: now,
  });
  await writeWorkspaceConfig({ dataModel: withProjectRow.dataModel });
  ...
  const { receipt } = await appendOutcomeReceipt({
    ...
    actor: "workspace-marketplace",
    objectRefs: [{ objectId: VERCEL_PROJECTS_OBJECT_ID, objectType: "custom", rowName: project.name }],
