---
name: growthub-video-use-studio-v1
description: "Self-contained workspace for conversational video editing with browser-use/video-use. Produces implementation-ready video edit plans and rendered segments. Use when the user says: \"video use\", \"browser-use video\", \"conversational video editing\". Session memory tracks edit plans, per-cut renders, and self-eval retries."
triggers:
  - video use studio
  - fork growthub-video-use-studio-v1
progressiveDisclosure: true
sessionMemory:
  path: .growthub-fork/project.md
selfEval:
  criteria:
    - Operator contract (workers/video-use-studio-operator/CLAUDE.md) read before any material change.
...
# Growthub Agent Worker Kit — Video Use Studio

Discovery entry + routing menu for the `growthub-video-use-studio-v1` worker kit. Family: `studio`. Agent contract: `workers/video-use-studio-operator/CLAUDE.md`.

## When to use this skill

When the user's intent matches any of the triggers above — or when an agent has been dropped into a fork whose `.growthub-fork/fork.json` declares `kitId: "growthub-video-use-studio-v1"`.
...
│         then: growthub kit download growthub-video-use-studio-v1 --out <path>
│
└── Yes → read in this order:
          ...
          4. workers/video-use-studio-operator/CLAUDE.md                              — agent contract
...
- `workers/video-use-studio-operator/CLAUDE.md` — agent contract
