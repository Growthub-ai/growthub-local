---
name: growthub-open-montage-studio-v1
description: "Self-contained workspace for OpenMontage agentic video production — end-to-end video content with EDL generation, segment rendering, and self-eval at every cut boundary. Use when the user says: \"open montage\", \"agentic video\", \"montage studio\". Session memory tracks EDL drafts, per-cut retries, and final assembly acceptance."
triggers:
  - open montage studio
  - fork growthub-open-montage-studio-v1
progressiveDisclosure: true
sessionMemory:
  path: .growthub-fork/project.md
selfEval:
  criteria:
    - Operator contract (workers/open-montage-studio-operator/CLAUDE.md) read before any material change.
...
# Growthub Agent Worker Kit — Open Montage Studio

Discovery entry + routing menu for the `growthub-open-montage-studio-v1` worker kit. Family: `studio`. Agent contract: `workers/open-montage-studio-operator/CLAUDE.md`.

## When to use this skill

When the user's intent matches any of the triggers above — or when an agent has been dropped into a fork whose `.growthub-fork/fork.json` declares `kitId: "growthub-open-montage-studio-v1"`.
...
│         then: growthub kit download growthub-open-montage-studio-v1 --out <path>
│
└── Yes → read in this order:
          ...
          4. workers/open-montage-studio-operator/CLAUDE.md                              — agent contract
...
- `workers/open-montage-studio-operator/CLAUDE.md` — agent contract
