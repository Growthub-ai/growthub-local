---
name: growthub-twenty-crm-v1
description: "Self-contained workspace for Twenty CRM — the open-source Salesforce/Apollo alternative. Produces CRM workflows, pipeline rules, and automation specs. Use when the user says: \"twenty crm\", \"crm workflows\", \"pipeline automation\". Session memory tracks workflow iterations, rule approvals, and migration checkpoints."
triggers:
  - twenty crm
  - fork growthub-twenty-crm-v1
progressiveDisclosure: true
sessionMemory:
  path: .growthub-fork/project.md
selfEval:
  criteria:
    - Operator contract (workers/twenty-crm-operator/CLAUDE.md) read before any material change.
...
# Growthub Agent Worker Kit — Twenty CRM

Discovery entry + routing menu for the `growthub-twenty-crm-v1` worker kit. Family: `studio`. Agent contract: `workers/twenty-crm-operator/CLAUDE.md`.

## When to use this skill

When the user's intent matches any of the triggers above — or when an agent has been dropped into a fork whose `.growthub-fork/fork.json` declares `kitId: "growthub-twenty-crm-v1"`.
...
│         then: growthub kit download growthub-twenty-crm-v1 --out <path>
│
└── Yes → read in this order:
          ...
          4. workers/twenty-crm-operator/CLAUDE.md                              — agent contract
...
- `workers/twenty-crm-operator/CLAUDE.md` — agent contract
