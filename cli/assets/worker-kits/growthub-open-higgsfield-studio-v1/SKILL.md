---
name: growthub-open-higgsfield-studio-v1
description: "Self-contained workspace for Open Higgsfield AI visual production using Muapi BYOK. Produces implementation-ready visual briefs + rendered assets. Use when the user says: \"higgsfield\", \"muapi\", \"ai visual studio\". Session memory tracks prompt iterations, generation costs, and self-eval outcomes per render."
triggers:
  - open higgsfield studio
  - fork growthub-open-higgsfield-studio-v1
progressiveDisclosure: true
sessionMemory:
  path: .growthub-fork/project.md
selfEval:
  criteria:
    - Operator contract (workers/open-higgsfield-studio-operator/CLAUDE.md) read before any material change.
...
# Growthub Agent Worker Kit — Open Higgsfield Studio

Discovery entry + routing menu for the `growthub-open-higgsfield-studio-v1` worker kit. Family: `studio`. Agent contract: `workers/open-higgsfield-studio-operator/CLAUDE.md`.

## When to use this skill

When the user's intent matches any of the triggers above — or when an agent has been dropped into a fork whose `.growthub-fork/fork.json` declares `kitId: "growthub-open-higgsfield-studio-v1"`.
...
│         then: growthub kit download growthub-open-higgsfield-studio-v1 --out <path>
│
└── Yes → read in this order:
          ...
          4. workers/open-higgsfield-studio-operator/CLAUDE.md                              — agent contract
...
- `workers/open-higgsfield-studio-operator/CLAUDE.md` — agent contract
