---
name: growthub-geo-seo-v1
description: "Self-contained workspace for GEO + SEO auditing using geo-seo-claude. Produces AI search visibility audits, technical SEO reports, and content gap analyses. Use when the user says: \"geo seo\", \"ai search visibility\", \"seo audit\". Session memory tracks crawled properties, audit runs, and recommendation-acceptance outcomes."
triggers:
  - geo seo
  - geo seo studio
  - fork growthub-geo-seo-v1
progressiveDisclosure: true
sessionMemory:
  path: .growthub-fork/project.md
selfEval:
  criteria:
    - Operator contract (workers/geo-seo-operator/CLAUDE.md) read before any material change.
...
# Growthub Agent Worker Kit — GEO SEO Studio

Discovery entry + routing menu for the `growthub-geo-seo-v1` worker kit. Family: `studio`. Agent contract: `workers/geo-seo-operator/CLAUDE.md`.

## When to use this skill

When the user's intent matches any of the triggers above — or when an agent has been dropped into a fork whose `.growthub-fork/fork.json` declares `kitId: "growthub-geo-seo-v1"`.
...
│         then: growthub kit download growthub-geo-seo-v1 --out <path>
│
└── Yes → read in this order:
          ...
          4. workers/geo-seo-operator/CLAUDE.md                              — agent contract
...
- `workers/geo-seo-operator/CLAUDE.md` — agent contract
