...
test("resources route: mode-aware account auth from the single provider contract", () => {
  assert.ok(resourcesRouteSource.includes("resolveProviderAccountAuth(provider"), "account auth resolves from the provider accountProbe contract");
  assert.ok(resourcesRouteSource.includes('providerAccountAuthMode(provider) === "bearer"'), "bearer mode branch exists");
  assert.ok(!resourcesRouteSource.includes("Basic ${Buffer.from") || resourcesRouteSource.includes("resolveProviderAccountAuth"), "Basic is no longer unconditional");
});
