...
test("marketplace surface renders live products through governed routes only", () => {
  ...
  assert.ok(source.includes("activeProductIsDiscovered"), "discovered products need their own install sidecar branch");
  assert.ok(source.includes("Ready to install integration"), "discovered install drawer must not use the generic resource install copy");
  assert.ok(source.includes("if (providerProductDiscovery) return discoveredProducts"), "live discovery providers must not render their static discovery contract as an install product");
  assert.ok(source.includes('["Installed products", String(installedProviderRows.length)]'), "details count must include discovered installed rows");
  assert.ok(source.includes("|| productLikeFromRow(row)"), "installed discovered rows must render instead of being dropped as unknown static products");
  assert.ok(!source.includes("unknown provider product"), "browser sidecar must not render server error copy as product configuration");
...
test("settings/apps shows the nango app icon only from verified governed api-registry rows", () => {
  const source = readKitFile("app/settings/apps/page.jsx");
  assert.ok(source.includes("listInstalledNangoProducts"), "nango app link derivation missing");
  assert.ok(source.includes('connectorKind || "").trim() === "nango"'), "apps card must key off governed nango registry rows");
  assert.ok(source.includes('syncStatus || "").trim() === "verified"'), "apps card nango icon must require verified proof");
  assert.ok(source.includes("/integrations/nango/integrations.png"), "apps card must render the nango integration icon");
  assert.ok(source.includes("providerConfigKey"), "apps card link should identify the installed Nango integration");
});
