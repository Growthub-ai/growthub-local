...
// ---------------------------------------------------------------------------
// Field type vocabulary — mirrors twenty-sdk/define field types
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// View vocabulary — mirrors twenty-sdk/define views
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Portal object definition — mirrors twenty-sdk/defineObject
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Portal capability — mirrors twenty-sdk dashboard tab + bindings
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Integration — mirrors the production `AgencyPortalIntegration` shape
// ---------------------------------------------------------------------------
 ...
 */
export type IntegrationSetupMode =
  | "hosted-authority"
  | "bring-your-own-key"
  | "local-catalog"
  | (string & {});
 ...
 */
export type IntegrationAuthPath =
  | "growthub-mcp-bridge"
  | "byo-api-key"
  | "local-catalog"
  | (string & {});
...
// ---------------------------------------------------------------------------
// Composition — top-level manifest a `growthub.config.ts` file ships
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Adapter selector — mirrors the production env-var driven shape
// ---------------------------------------------------------------------------
 ...
 */
export function definePortalCapability<T extends PortalCapability>(spec: T): T {
  return spec;
}
 ...
 */
export function definePortalObject<T extends PortalObjectDefinition>(spec: T): T {
  return spec;
}
 ...
 */
export function defineIntegration<T extends PortalIntegration>(spec: T): T {
  return spec;
}
 ...
 * single subpath.
 */
export function defineWidget<T extends WidgetDefinition>(spec: T): T {
  return spec;
}
 ...
 */
export function defineCanvas<T extends CanvasDefinition>(spec: T): T {
  return spec;
}
 ...
 * ```
 */
export function defineComposition<T extends Composition>(spec: T): T {
  return spec;
}
 ...
 */
export function groupIntegrationsByLane<T extends PortalIntegration>(
  integrations: ReadonlyArray<T>,
): GroupedIntegrations<T> {
  return {
    dataSources: integrations.filter((item) => item.lane === "data-source"),
    workspaceIntegrations: integrations.filter(
      (item) => item.lane === "workspace-integration",
    ),
  };
}
 ...
 */
export const COMPOSITIONS_CONTRACT_VERSION = 1 as const;
