...
export const WORKER_KIT_FAMILIES: ReadonlyArray<"studio" | "workflow" | "operator" | "ops"> =
  ["studio", "workflow", "operator", "ops"] as const;

export type WorkerKitVisibility =
  | "public-open-source"
  | "public"
  | "private"
  | "internal"
...
// ---------------------------------------------------------------------------
// Compatibility / install / UI / provenance — v2 metadata blocks
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Manifest — v1 + v2 discriminated union
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Bundle sidecar — v1 + v2 discriminated union
// ---------------------------------------------------------------------------
...
export function isWorkerKitManifestV2(
  manifest: WorkerKitManifest,
): manifest is WorkerKitManifestV2 {
  return manifest.schemaVersion === 2;
}

export function isWorkerKitManifestV1(
  manifest: WorkerKitManifest,
): manifest is WorkerKitManifestV1 {
  return manifest.schemaVersion === 1;
}

export function isWorkerKitBundleManifestV2(
  manifest: WorkerKitBundleManifest,
): manifest is WorkerKitBundleManifestV2 {
  return manifest.schemaVersion === 2;
}

export function isWorkerKitBundleManifestV1(
  manifest: WorkerKitBundleManifest,
): manifest is WorkerKitBundleManifestV1 {
  return manifest.schemaVersion === 1;
}
 ...
 * kits.
 */
export function isAppKit(manifest: WorkerKitManifest): boolean {
...
export const WORKER_KIT_SUPPORTED_SCHEMA_VERSIONS = [1, 2] as const;

/** Latest schema version (every kit on `main` declares this). */
export const WORKER_KIT_LATEST_SCHEMA_VERSION = 2 as const;
